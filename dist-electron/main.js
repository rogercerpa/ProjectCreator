...
let mainWindow;
let activeUploads = /* @__PURE__ */ new Set();
...
const ECD_NOTIFY_THROTTLE_MS = 6 * 60 * 60 * 1e3;
const lastECDNotifyByRfa = /* @__PURE__ */ new Map();
...
function createWindow() {
  mainWindow = new BrowserWindow({
    ...
    webPreferences: {
      ...
      // Prevent IPC/timer throttling during idle periods
...
app.on("before-quit", async () => {
  try {
    ...
    console.warn("Failed to stop shared calendar watcher:", (error == null ? void 0 : error.message) || error);
  }
});
app.on("activate", () => {
  ...
  try {
    ...
    if (!(result == null ? void 0 : result.success)) {
      await errorLogService.logError({
        ...
        userMessage: `Failed to download folder: ${(result == null ? void 0 : result.error) || "Unknown error"}`,
        technicalMessage: result == null ? void 0 : result.error,
        context: { projectId: project == null ? void 0 : project.id, projectName: project == null ? void 0 : project.projectName, zipFilePath }
      });
    }
    return result;
  } catch (error) {
  ...
  try {
    ...
    if (!result.success && !result.needsConfirmation) {
      await errorLogService.logError({
        ...
        context: { projectId: project == null ? void 0 : project.id, projectName: project == null ? void 0 : project.projectName }
      });
    }
    return result;
  } catch (error) {
...
ipcMain.handle("agile-start-monitoring", async (event, intervalMs) => {
  try {
    agileScrapingService.startMonitoring(intervalMs ?? 5 * 60 * 1e3);
    return { success: true };
  } catch (e) {
    return { success: false, error: e.message };
  }
...
ipcMain.handle("agile-scrape-now", async () => {
  try {
    ...
    return { entries: [], scrapedAt: (/* @__PURE__ */ new Date()).toISOString(), error: e.message };
  }
});
...
ipcMain.handle("agile-download-document", async (event, { url, filename }) => {
  try {
    const { filePath } = await dialog.showSaveDialog(mainWindow || void 0, {
...
ipcMain.handle("error-log-export", async () => {
  try {
    ...
    const { canceled, filePath } = await dialog.showSaveDialog(mainWindow || void 0, {
...
ipcMain.handle("oneDriveSyncUpload", async (event, { projectPath, projectData, settings }) => {
  const uploadId = `onedrive-${(projectData == null ? void 0 : projectData.id) || "unknown"}-${Date.now()}`;
  try {
    activeUploads.add(uploadId);
